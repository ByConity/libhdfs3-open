...
DatanodeImpl::DatanodeImpl(const std::string & host, uint32_t port, const std::string & ip,
                           const SessionConfig & c, const RpcAuth & a) :
    auth(a), client(RpcClient::getClient()), conf(c), protocol(
        DATANODE_VERSION, DATANODE_PROTOCOL, BLOCK_TOKEN_KIND), server(host, ip, port) {
    server.setTokenService("");
}
