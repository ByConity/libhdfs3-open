...
static void handleException(Hdfs::exception_ptr error) {
    try {
        ...
        // More informational log for HDFS related debug
//#ifndef NDEBUG
        std::string buffer;
...
std::string hdfsBuilderGetNameNode(struct hdfsBuilder *bld)
{
    assert(bld != NULL);
    return bld->nn;
}
...
hdfsFile hdfsOpenFile(hdfsFS fs, const char * path, int flags, int bufferSize,
                      short replication, tOffset blocksize, hdfsEventCallBack cb) {
    LOG_TIME("hdfsOpenFile");
    ...
    try {
        ...
        if ((flags & O_CREAT) || (flags & O_APPEND) || (flags & O_WRONLY)) {
            ...
            file->setInput(true);
            is = new InputStream;
            is->open(fs->getFilesystem(), path, true,cb);
            file->setStream(is);
        }
...
int hdfsExists(hdfsFS fs, const char * path) {
    ...
    try {
        return fs->getFilesystem().exist(path) ? 0 : ENOENT;
    } catch (const std::bad_alloc & e) {
        SetErrorMessage("Out of memory");
        errno = ENOMEM;
    } catch (...) {
...
tSize hdfsPRead(hdfsFS fs, hdfsFile file, void * buffer, tSize length) {
//    LOG_TIME("hdfsPRead");
    LOG_TIME_LV(Hdfs::Internal::DEBUG2, "hdfsPRead");
    PARAMETER_ASSERT(fs && file && buffer && length > 0, -1, EINVAL);
    PARAMETER_ASSERT(file->isInput(), -1, EINVAL);
    try {
        return file->getInputStream().pread(static_cast<char *>(buffer), length);
    } catch (const Hdfs::HdfsEndOfStream & e) {
        return 0;
    } catch (const std::bad_alloc & e) {
        SetErrorMessage("Out of memory");
...
int hdfsDelete(hdfsFS fs, const char * path, int recursive) {
    ...
    try {
        int ret = fs->getFilesystem().deletePath(path, recursive) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
int hdfsRename(hdfsFS fs, const char * oldPath, const char * newPath) {
    ...
    try {
        int ret = fs->getFilesystem().rename(oldPath, newPath) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
char * hdfsGetWorkingDirectory(hdfsFS fs, char * buffer, size_t bufferSize) {
    LOG_TIME("hdfsGetWorkingDirectory");
...
int hdfsSetWorkingDirectory(hdfsFS fs, const char * path) {
    LOG_TIME("hdfsSetWorkingDirectory");
...
int hdfsCreateDirectory(hdfsFS fs, const char * path) {
    LOG_TIME("hdfsCreateDirectory");

    PARAMETER_ASSERT(fs && path && strlen(path) > 0, -1, EINVAL);

    try {
        int ret = fs->getFilesystem().mkdirs(path, 0755) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
int hdfsCreateDirectoryEx(hdfsFS fs, const char * path, short mode, int createParents) {
    LOG_TIME("hdfsCreateDirectoryEx");
    ...
    try {
        int ret = 0;
        if (createParents)
            ret = fs->getFilesystem().mkdirs(path, mode) ? 0 : -1;
        else
            ret = fs->getFilesystem().mkdir(path, mode) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
int hdfsSetReplication(hdfsFS fs, const char * path, int16_t replication) {
    LOG_TIME("hdfsSetReplication");

    PARAMETER_ASSERT(fs && path && strlen(path) > 0 && replication > 0, -1, EINVAL);

    try {
        int ret = fs->getFilesystem().setReplication(path, replication) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
tOffset hdfsGetDefaultBlockSize(hdfsFS fs) {
    LOG_TIME("hdfsGetDefaultBlockSize");
...
char * hdfsGetDelegationToken(hdfsFS fs, const char * renewer) {
    LOG_TIME("hdfsGetDelegationToken");
...
int64_t hdfsRenewDelegationToken(hdfsFS fs, const char * token) {
    LOG_TIME("hdfsRenewDelegationToken");
...
int hdfsCancelDelegationToken(hdfsFS fs, const char * token) {
    LOG_TIME("hdfsCancelDelegationToken");
...
BlockLocation * hdfsGetFileBlockLocations(hdfsFS fs, const char * path,
    ...
    LOG_TIME("hdfsGetFileBlockLocations");
...
int hdfsCreateEncryptionZone(hdfsFS fs, const char * path, const char * keyName) {
    LOG_TIME("hdfsCreateEncryptionZone");

    PARAMETER_ASSERT(fs && path && strlen(path) > 0 && keyName && strlen(keyName) > 0, -1, EINVAL);

    try {
        int ret = fs->getFilesystem().createEncryptionZone(path, keyName) ? 0 : -1;
        if (ret == -1) {
            errno = HDFS_EFAILED;
        }
        return ret;
...
hdfsEncryptionZoneInfo * hdfsListEncryptionZones(hdfsFS fs, int * numEntries) {
    LOG_TIME("hdfsListEncryptionZones");
