...
public:
    ExtendedBlock() :
        blockId(0), blockPufsName(""), generationStamp(0), numBytes(0) {
    }
    ...
    const std::string & getBlockPufsName() const {
        return blockPufsName;
    }

    void setBlockPufsName(const std::string & blockPufsName) {
        this->blockPufsName = blockPufsName;
    }

    const std::string toString() const {
        std::stringstream ss;
        ss << "[block pool ID: " << poolId
           << " block pufs name " << blockPufsName
           << " block ID " << blockId << "_"
           << generationStamp << "]";
        return ss.str();
    }
