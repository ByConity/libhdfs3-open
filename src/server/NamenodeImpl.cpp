...
NamenodeImpl::NamenodeImpl(const char * host, const char * port, const char * ip, const std::string & tokenService,
                           const SessionConfig & c, const RpcAuth & a) :
    auth(a), client(RpcClient::getClient()), conf(c), protocol(
        NAMENODE_VERSION, NAMENODE_PROTOCOL, DELEGATION_TOKEN_KIND), server(tokenService, host, ip, port),
        sessionConfig(c) {
}
...
void NamenodeImpl::invoke(RpcCall call) {
    ...
    try {
        ...
        channel.close(false);
        LOG(LOG_ERROR, "Method: %s Request: %s Response: %s", call.getName(),
            call.getRequest()->ShortDebugString().c_str(), call.getResponse()->ShortDebugString().c_str());
        throw;
    }
