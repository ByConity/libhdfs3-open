...
public:
    ...
    RpcServerInfo(const std::string & tokenService, const std::string & h, const std::string & p) :
        host(h), ip(""), port(p), tokenService(tokenService) {
    }

    RpcServerInfo(const std::string & tokenService, const std::string & h, const std::string & ip, const std::string & p) :
        host(h), ip(ip), port(p), tokenService(tokenService) {
    }
    ...
    RpcServerInfo(const std::string & h, const std::string & ip, uint32_t p) :
        host(h), ip(ip) {
        std::stringstream ss;
        ss.imbue(std::locale::classic());
        ss << p;
