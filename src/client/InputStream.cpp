...
InputStream::InputStream() {
    impl = std::make_shared<InputStreamImpl>();
}
...
void InputStream::open(FileSystem & fs, const char * path,
    ...
    impl->open(fs.impl->filesystem, path, verifyChecksum,cb);
}
 ...
 * @return return the number of bytes filled in the buffer, it may less than size.
 */
int32_t InputStream::pread(char * buf, int32_t size) {
    return impl->pread(buf, size);
}
