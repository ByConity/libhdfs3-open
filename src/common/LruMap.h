...
public:
    ...
    typedef std::pair<K, V> ItmeType;
    typedef std::list<ItmeType> ListType;
    typedef std::multimap<K, typename ListType::iterator> MapType;

public:
    LruMultiMap() : count(0), maxSize(1000) { }

    LruMultiMap(size_t size) : count(0), maxSize(size) { }
    ...
    void insert(const KeyType & key, const ValueType & value)
    {
        ...
        // Since we use multimap, so we will not erase the duplicate key.
        list.push_front(std::make_pair(key, value));
        map.emplace(std::make_pair(key, list.begin()));
        ++count;

        if (count > maxSize)
        {
            auto range = map.equal_range(list.back().first);
            auto last = list.end();
            --last;
...
private:
    bool findAndEraseInternal(const KeyType & key, ValueType * value, bool erase)
    {
        typename MapType::iterator it = map.find(key);
