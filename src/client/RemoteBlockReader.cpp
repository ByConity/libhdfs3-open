...
RemoteBlockReader::RemoteBlockReader(shared_ptr<FileSystemInter> filesystem,
                                     ...
                                     SessionConfig& conf,
                                     std::shared_ptr<FastSwitchReadStrategy> fast_failover_strategy,
                                     hdfsEventCallBack hdfsEventCallback)
    : sentStatus(false),
      verify(verify),
      binfo(eb),
      datanode(datanode),
      ...
      conf(conf),
      hdfs_event_callback(hdfsEventCallback){
//    LOG(INFO, "construct %d %d", cursor,endOffset);
    assert(start >= 0);
    ...
    try {
        ...
        if (fast_failover_strategy) {
            fast_failover_strategy->resetWindow();
            sock->setSwitchReadStrategy(fast_failover_strategy);
        }
...
void RemoteBlockReader::setupReader(SessionConfig& conf)
{
    ...
    sender = shared_ptr<DataTransferProtocol>(new DataTransferProtocolSender(
        *sock, writeTimeout, datanode.formatAddress(), conf.getEncryptedDatanode(),
        ...
        conf.getUseInternalCredentialFormat()));
    reader = shared_ptr<DataReader>(new DataReader(sender.get(), in, readTimeout));
}

void RemoteBlockReader::cleanupSocket() {
    if(conf.getEnableSocketCache()){
        bool reuse = sentStatus;
        if (reuse) {
            peerCache.addConnection<Socket>(sock, datanode);
        } else {
            LOG(DEBUG2,"PeerCache close");
            sock->close();
        }
    }else {
        sock->close();
...
shared_ptr<Socket> RemoteBlockReader::getNextPeer(const DatanodeInfo& dn) {
    LOG_TIME("dn.connect.timecost");
    shared_ptr<Socket> sock;
    try {
        if(conf.enableSocketCache)
        {
            sock = peerCache.getConnection<Socket>(dn);
        }
        if (!sock) {
...
void RemoteBlockReader::readNextPacket() {
    ...
    // EMIT_TIME_AND_LOG("read.packet.latency", "read.packet");
...
//    LOG(WARNING, "%s curors: %d endoffset: %d", __FUNCTION__ , cursor,endOffset);
    ...
    try {
        if (position >= size) {
            Stopwatch sw;
            SCOPE_EXIT({
                CNCH_HDFS_CALLBACK2(hdfs_event_callback, Event::HDFS_EVENT_READ_PACKET,int64_t(sw.elapsedNanoseconds()));
            });
            readNextPacket();
        }
