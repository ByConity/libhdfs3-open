...
static inline void BuildBaseHeader(const ExtendedBlock & block,
    ...
    eb->set_blockpufsname(block.getBlockPufsName());
...
static inline void BuildNodeInfo(const DatanodeInfo & node,
    ...
    for (auto & optIpAddr : node.getOptIpAddrs()) {
        auto * ipAddr = id->add_optipaddrs();
        *ipAddr = optIpAddr;
    }
...
DataTransferProtocolSender::DataTransferProtocolSender(Socket & sock,
    ...
    protection(protection), useInternalCredentialFormat(useInternalCred) {
}
...
void DataTransferProtocolSender::setupSasl(const ExtendedBlock blk, const Token& blockToken) {
    ...
    saslClient = new SaslClient(auth, ourToken, "", isSecure, protection, useInternalCredentialFormat);
...
{
    void composeReadBlockHeader(
        const ExtendedBlock & blk, const Token & blockToken, const char * clientName, int64_t blockOffset, int64_t length, WriteBuffer * wb)
    {
        OpReadBlockProto op;
        op.set_len(length);
        op.set_offset(blockOffset);
        ...
        wb->writeBigEndian(static_cast<int16_t>(DATA_TRANSFER_VERSION));
        wb->write(static_cast<char>(READ_BLOCK));
        int msgSize = op.ByteSize();
        wb->writeVarint32(msgSize);
        char * b = wb->alloc(msgSize);
        if (!op.SerializeToArray(b, msgSize))
        {
            THROW(HdfsIOException, "DataTransferProtocolSender cannot serialize header to send buffer.");
        }
    }
}
