...
BufferedSocketReaderImpl::~BufferedSocketReaderImpl() noexcept
{
}
...
int32_t BufferedSocketReaderImpl::readBigEndianInt32(int timeout) {
    ...
    int32_t buf[1];
    readFully(reinterpret_cast<char*>(buf), sizeof(buf), timeout);
...
int32_t BufferedSocketReaderImpl::readVarint32(int timeout, int32_t step) {
    ...
    while (!rc) {
        ...
        if (timeout >= 0 && deadline <= 0) {
            // Add debug information about socket target
            std::string socketInfo = "";
            if (auto tcpSockPtr = dynamic_cast<TcpSocketImpl *>(&sock))
            {
                socketInfo = tcpSockPtr->getRemoteAddr();
            }
            THROW(HdfsTimeoutException, "Read %d bytes timeout %s", size, socketInfo.c_str());
        }
    }
