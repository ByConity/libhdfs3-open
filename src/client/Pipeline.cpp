...
void PipelineImpl::transfer(const ExtendedBlock & blk, const DatanodeInfo & src,
    ...
    DataTransferProtocolSender sender2(*so, writeTimeout, src.formatAddress(), config.getEncryptedDatanode(),
        ...
        config.getUseInternalCredentialFormat());
            ...
            if (stage != PIPELINE_SETUP_CREATE && stage != PIPELINE_CLOSE
                ...
                if (nodes.empty() && useRemoved) {
                    if (storageID.length()) {
                        ...
                        lb->setBlockPufsName(lastBlock->getBlockPufsName());
 ...
 */
std::string PipelineImpl::checkBadLinkFormat(const std::string & n) {
    std::string node = n;
...
void PipelineImpl::createBlockOutputStream(const Token & token, int64_t gs, bool recovery) {
    ...
    try {
        ...
        sender = shared_ptr<DataTransferProtocolSender>(new DataTransferProtocolSender(*sock, writeTimeout,
                                          ...
                                          config.getDataProtection(), config.getUseInternalCredentialFormat()));
    ...
    firstBadLink = checkBadLinkFormat(firstBadLink);
