...
std::once_flag peerCacheInitOnce;
std::shared_ptr<PeerCache> peerCacheSingleton;
      ...
      user(key.getUser())
{
    ...
    {
        std::call_once(peerCacheInitOnce, [this] { 
            peerCacheSingleton = std::make_shared<PeerCache>(this->sconf);
            auto & blockLocationManager = Hdfs::Internal::BlockLocationManager::GetSingleton(sconf.getBlockLocationCacheSize());
            (void)blockLocationManager; 
             });
    }
    assert(peerCacheSingleton !=nullptr);
    peerCache = peerCacheSingleton;
    nodeResolver = shared_ptr<NodeResolver>(new NodeResolver(sconf));
#ifdef MOCK
    stub = NULL;
#endif
    //set log level
...
void FileSystemImpl::connect() {
    ...
    LOG(DEBUG2, "FsImpl Connect to <%s>://<%s>:%s", key.getScheme().c_str(), key.getHost().c_str(), key.getPort().c_str());
    ...
    nn = stub->getNamenode();
#else
    nn = new NamenodeProxy(namenodeInfos, tokenService, sconf,
