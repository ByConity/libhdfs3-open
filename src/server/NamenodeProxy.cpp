...
NamenodeProxy::NamenodeProxy(const std::vector<NamenodeInfo> & namenodeInfos, const std::string & tokenService,
    ...
    for (size_t i = 0; i < namenodeInfos.size(); ++i) {
        ...
        if (nninfo.size() != 2) {
            /// Try to transalte according ipv6
            auto & rpcAddr = namenodeInfos[i].getRpcAddr();
            if (!rpcAddr.empty() && rpcAddr[0] == '[')
            {
                nninfo.clear();
                int64_t j = 0;
                for (;  j < rpcAddr.size() - 1; ++j)
                {
                    if (rpcAddr[j] == ']' and rpcAddr[j + 1] == ':')
                        break;
                }

                if (j > 0 && j < rpcAddr.size() - 2)
                {
                    nninfo.push_back(rpcAddr.substr(1, j - 1));
                    nninfo.push_back(rpcAddr.substr(j + 2, rpcAddr.size() - (j + 2)));
                }
            }
        }
        ...
        namenodes.push_back(
            shared_ptr<Namenode>(
                new NamenodeImpl(nninfo[0].c_str(), nninfo[1].c_str(), namenodeInfos[i].getIp().c_str(), clusterid, c, a)));
    }
