...
static inline void Convert(ExtendedBlock & eb,
    ...
    eb.setBlockPufsName(proto.blockpufsname());
}
...
static inline void Convert(DatanodeInfo & node,
    ...
    for (auto ipAddr : idProto.optipaddrs()) {
        optIpAddrs.push_back(ipAddr);
    }
    node.setOptIpAddrs(optIpAddrs);
...
static inline void Build(const DatanodeInfo & dn, DatanodeIDProto * proto) {
    ...
    for (auto & optIpAddr : dn.getOptIpAddrs()) {
        auto * ipAddr = proto->add_optipaddrs();
        *ipAddr = optIpAddr;
    }
}
