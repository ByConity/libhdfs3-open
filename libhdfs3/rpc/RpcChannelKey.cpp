...
#include "cpputil/infsec/client/InfSecClient.h"
...
RpcChannelKey::RpcChannelKey(const RpcAuth & a, const RpcProtocolInfo & p,
    ...
    if (enableInfsec()){
        // GET RIGHT TOKEN
        std::string tokenStr = infsec::client::InfSecClient::getInstance().getToken();
        Token * temp = new Token();
        temp->setIdentifier(tokenStr);
        temp->setPassword(tokenStr);
        token = shared_ptr<Token> (temp);
    } else {
