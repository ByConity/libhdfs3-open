...
void SaslClient::initDigestMd5(const RpcSaslProto_SaslAuth & auth,
    ...
    std::string hsasl_identify;
    hsasl_put_int_to_bytes(hsasl_identify, int(this->saslToken.size()));
    hsasl_identify.append(this->saslToken);
    hsasl_identify = hsasl_base64_encode(hsasl_identify);
    std::string hsasl_password = hsasl_base64_encode(this->saslToken);

    gsasl_property_set(session, GSASL_AUTHID, hsasl_identify.c_str());
    gsasl_property_set(session, GSASL_PASSWORD, hsasl_password.c_str());
    gsasl_property_set(session, GSASL_HOSTNAME, auth.serverid().c_str());
    gsasl_property_set(session, GSASL_SERVICE, auth.protocol().c_str());
    changeLength = true;
