...
void TcpSocketImpl::readFully(char * buffer, int32_t size, int timeout) {
    ...
    while (todo > 0) {
        ...
        {
            polled = poll(true, false, deadline);
        }
        if (polled) {
        ...
        if (switch_read_strategy_ && rc > 0) {
            switch_read_strategy_->recordThroughput(rc, s, e); // will throw exception on slow read.
            rc = 0;
        }
...
void TcpSocketImpl::setSwitchReadStrategy(
    std::shared_ptr<FastSwitchReadStrategy> switch_read_strategy) {
    switch_read_strategy_ = switch_read_strategy;
}
